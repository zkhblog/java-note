...
①消息存储时采用顺序写的方式，保证了消息存储的速度。（顺序写比随机写快多个数量级）

②消息发送时，通过零拷贝机制省去了向用户态的内存复制，采用MappedByteBuffer这种内存映射的方式提高了速度。（内存映射时一次只能映射1.5G-2G的文件，这也是RocketMQ默认设置单个CommitLog日志数据文件为1G的原因）
