...
### 3、分析器
先做“词法分析”，识别出SQL语句里面的字符串分别代表什么  
再做“语法分析”，判断你输入的SQL语句是否满足MySQL语法  

### 4、优化器
决定执行方案。有多个索引的时候，决定使用哪个索引；多表关联的时候，决定各表的关联顺序。

### 5、执行器
开始执行的时候，判断当前连接是否有对这个表的查询权限。如果没用，则返回没用权限的错误。  
...
## 两阶段提交
①跨系统维持数据逻辑一致性时的常用方案；  
②两阶段提交可以保证数据库的状态和用它的日志（binlog）恢复出来的库的状态一致。
如果redo log丢失，崩溃恢复以后，这个事务无效，原库值不变，但是用binlog恢复的临时库多了一次更新。
binlog丢失，用这个binlog恢复临时库的话，临时库少一次更新。而redo log能保证异常重启后数据是更新后的状态，这会导致出现主从数据库不一致的情况。
